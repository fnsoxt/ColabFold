...
digits_chain ='ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'
digits_chain_values = dict([pair for pair in zip(digits_chain, range(62))])

PDB_MAX_CHAINS = 3844

def encode_chain_id(value):
    if value>=3844:
        raise ValueError("value out of chain id range.")
    return encode_pure(digits_chain, value).rjust(2)

def decode_chain_id(s):
    return decode_pure(digits_chain_values, s.strip())

def encode_atom_id(value):
    return hy36encode(5, value)

def decode_atom_id(s):
    return hy36decode(5, s)


def encode_res_id(value):
    return hy36encode(4, value)

def decode_res_id(s):
    return hy36decode(4, s)


def encode_sse_id(value):
    return hy36encode(3, value)

def decode_sse_id(s):
    return hy36decode(3, s)
